...
int lines[15] = {12, 13, 14, 15, 16, 23, 24, 25, 26, 34, 35, 36, 45, 46, 56};
// Posible lines according to above sequence : 12, 13, 14, 15, 16, 23, 24, 25, 26, 34, 35, 36, 45, 46, 56

typedef char board_t[15];
typedef char player_t; /* A player should be RED or BLUE. */

int random_range_num(int min, int max)
{
    return min + rand() % (max - min + 1); // Getting randi int
}
...
int find_max_index(int array[], int size)
{
    if (size <= 0)
    {
        return -1; // This is a invalid size
    }
    ...
    for (int i = 1; i < size; i++)
    {
        ...
        {
            maximum_index = i; // Updating maximum_index if a larger element is found while looping.
        }
    }
...
int has_won(board_t board, player_t player)
{
    ...
    while (i < 20)
    {
        int a = valid_triangle[i][0];
        int b = valid_triangle[i][1];
        int c = valid_triangle[i][2];
        // Looping to check for valid triangles.
...
move_t best_move(board_t board, player_t player)
{
    ...
    for (int i = 0; i < 15; i++)
    {
        if (board[i] == NO)
        {
            ...
            move_t move_oppo = best_move(board, opponent);
            int score = move_oppo.score;
...
int main()
{
    ...
    while (1)
    {
        printf("Select Color First (R) or Second (B)? ");
    ...
    // Now Checking for the winner.
    player_t winner = NO;

    if (player == BLUE)
    {
        int line;
        line = random_range_num(0, 14);
        board[line] = computer;
    ...
    while (!is_full(board))
    {
        ...
        while (1)
        {
            ...
            scanf("%d", &x);
            move = find_index(lines, 15, x);

            if (move == -1)
            {
                printf("Invalid Input.Refer exapmple, 12,13,23...56\n\n");
            }
            else if (board[move] != NO)
            {
                printf("Vertices already connected! Please choose Valid move.\n\n");
            }
            else
            {
                break;
